...
def main():
    ...
    satellites = requests.get("http://www.celestrak.com/NORAD/elements/noaa.txt")
    ...
    with open("sat_names.txt", "w") as f:
         new_line_sats = [f"{sat}\n" for sat in satellitenames]
         f.writelines(new_line_sats)

    with open("sat_names.txt","r") as f2:
         txt = f2.read()
